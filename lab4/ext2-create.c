...
void write_block_bitmap(int fd) {
    ...
    const int TOTAL_BITS = BLOCK_SIZE * 8;
    block_bitmap[0] |= (1 << 0);
    assert((block_bitmap[0] & (1 << 0)) != 0);

    for (int i = SUPERBLOCK_BLOCKNO; i < LAST_BLOCK; i++) {
        int byte_offset = i / 8;
        int bit_offset = i % 8;
        block_bitmap[byte_offset] |= (1 << bit_offset);
        assert((block_bitmap[byte_offset] & (1 << bit_offset)) != 0);
    }

    for (int i = NUM_BLOCKS - 1; i < TOTAL_BITS; i++) {
        int byte_offset = i / 8;
        int bit_offset = i % 8;
        block_bitmap[byte_offset] |= (1 << bit_offset);
        assert((block_bitmap[byte_offset] & (1 << bit_offset)) != 0);
...
void write_inode_bitmap(int fd) {
    ...
    memset(inode_bitmap, 0xFF, BLOCK_SIZE);
    ...
    for (int i = 14; i <= 128; i++) {
        ...
        assert((inode_bitmap[byte_offset] & (1 << bit_offset)) == 0);
    }
...
void write_inode_table(int fd) {
    ...
    hello_symlink_inode.i_size = strlen("hello-world");
